...
//----------------------------------------------------------------------------------------------------------------------------------
void load_global_param(vector<double>& u_global_t,vector<double>& u_g_t,vector<double>& Fext_global_t,vector<double>& Fext_g_t,
...
//----------------------------------------------------------------------------------------------------------------------------------
void update_ctrl_pt(vector<double>& ctrlpt_coordinates,vector<double> delta_u_g_t1,int no_unrestrained_dof,vector<int> unrestrained_dof_data)
{
    for(int i=0;i<no_unrestrained_dof;i++){
        int subscript=unrestrained_dof_data[2*i]-1;
        int subscript_dof=unrestrained_dof_data[2*i+1];
        ctrlpt_coordinates[4*subscript+subscript_dof]+=delta_u_g_t1[i];
    }
}
