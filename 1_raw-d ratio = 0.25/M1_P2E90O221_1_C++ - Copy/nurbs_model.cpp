...
//  ********************************************************
void calc_nurbs_mod_prop(int p,int q,int r,int n,int m,int l,int& nel,int& nnp,int& nen,int& noctrlpt, int& noxi,int& noeta,
                         int& nozeta,int& nophyspts,vector<double> Xi,vector<double> Eta,vector<double> Zeta)
{
    double counter_1=0;
        ...
        double counter_3=0;
        for(int i=0;i<Zeta.size()-1;i++)
            {
            if(Zeta[i]==Zeta[i+1]){counter_3=counter_3;}
            else{counter_3++;}
            }

    nel=counter_1*counter_2*counter_3;
    //nel=(n-p)*(m-q)*(l-r);
    nnp=n*m*l;
    nen=(p+1)*(q+1)*(r+1);
    noctrlpt=n*m*l;
...
void build_NURBS_coords(vector<double> Xi,vector<double> Eta,vector<double> Zeta,vector<int>& NURBS_coords)
{
    int element = 0;

    vector<double> vector_xi;
    vector<double> vector_eta;
    vector<double> vector_zeta;
    ...
    //cout<<vector_eta.size()<<endl;

    for(int i=0;i<Zeta.size()-1;i++)
    {
        if(Zeta[i]!=Zeta[i+1]){vector_zeta.push_back(i);}
    }

    //for(int i=0;i<vector_zeta.size();i++){cout<<vector_zeta[i]<<endl;}

    NURBS_coords.resize(vector_xi.size()*vector_eta.size()*vector_zeta.size()*3);
    for(int k=0;k<vector_zeta.size();k++)
    {
        for(int j=0;j<vector_eta.size();j++)
        {
            ...
            {
                element++;
                NURBS_coords[3*(element-1)]=vector_xi[i];
                NURBS_coords[3*(element-1)+1]=vector_eta[j];
     ...
     int no_basis_funcs)
{
    ...
    B_spline_deriv.clear();
        for(int i=0;i<order+1;i++){
            ...
            if(knot_pt_val==1){
              if((knot_pt_index-order+i)==knot_pt_index){B_spline.push_back(1);}
              if(((knot_pt_index-order+i)<knot_pt_index)){B_spline.push_back(0);}
              B_spline_deriv.push_back(calc_Bspline_deriv(knot_pt_index-1,0.99999999,knot_vect,no_basis_funcs,order,i));
            }
            else{
              B_spline.push_back(calc_basis_func(knot_pt_index-order+i,knot_pt_val,knot_vect,no_basis_funcs,order));
...
void Shape_func_1(vector<int> ctrlpt_incidence,vector<double> ctrlpt_coordinates, vector<double>& NURBS_bas_func,vector<double>& NURBS_deriv,
                  vector<double> basis_func_N,vector<double> basis_func_M,vector<double> basis_func_L,vector<double> basis_deriv_N,
                  vector<double> basis_deriv_M,vector<double> basis_deriv_L,int order_p,int order_q,int order_r,int element,int nen,
                  int deg_of_freedom,vector<double>& dR_dx,vector<double>& dx_dxi)
{
    int loc_num=0;
    double sumtot=0;
    double sumxi=0;
    ...
    //e = element-1;
            ...
            for(int i=0;i<order_p+1;i++){
                ...
                NURBS_bas_func.push_back(basis_func_N[i]*basis_func_M[j]*basis_func_L[k]*ctrlpt_coordinates[4*(cc-1)+3]);
                sumtot+=NURBS_bas_func[loc_num-1];

                NURBS_deriv.push_back(basis_deriv_N[i]*basis_func_M[j]*basis_func_L[k]*ctrlpt_coordinates[4*(cc-1)+3]);
                sumxi+=NURBS_deriv[(loc_num-1)*deg_of_freedom];

                NURBS_deriv.push_back(basis_func_N[i]*basis_deriv_M[j]*basis_func_L[k]*ctrlpt_coordinates[4*(cc-1)+3]);
                sumeta+=NURBS_deriv[(loc_num-1)*deg_of_freedom+1];

                NURBS_deriv.push_back(basis_func_N[i]*basis_func_M[j]*basis_deriv_L[k]*ctrlpt_coordinates[4*(cc-1)+3]);
                sumzeta+=NURBS_deriv[(loc_num-1)*deg_of_freedom+2];
            ...
            for(int a=0;a<loc_num;a++){
                ...
                NURBS_deriv[deg_of_freedom*a+2]=(NURBS_deriv[deg_of_freedom*a+2]*sumtot-NURBS_bas_func[a]*sumzeta)/pow(sumtot,2);
            }

        //for(int i=0;i<nen;i++)
        //{
            //cout<<NURBS_deriv[3*i]<<" "<<NURBS_deriv[3*i+1]<<" "<<NURBS_deriv[3*i+2]<<endl;
        //}
        ...
        loc_num=0;
            ...
            determinant =dx_dxi[0]*(dx_dxi[4]*dx_dxi[8] - dx_dxi[7]*dx_dxi[5])
                                       -dx_dxi[1]*(dx_dxi[3]*dx_dxi[8] - dx_dxi[6]*dx_dxi[5])
                                       +dx_dxi[2]*(dx_dxi[3]*dx_dxi[7] - dx_dxi[6]*dx_dxi[4]);

                dxi_dx[0]=(dx_dxi[4]*dx_dxi[8] - dx_dxi[7]*dx_dxi[5])/determinant;
                dxi_dx[3]=-(dx_dxi[3]*dx_dxi[8] - dx_dxi[6]*dx_dxi[5])/determinant;
                dxi_dx[6]=(dx_dxi[3]*dx_dxi[7] - dx_dxi[6]*dx_dxi[4])/determinant;

                dxi_dx[1]=-(dx_dxi[1]*dx_dxi[8] - dx_dxi[7]*dx_dxi[2])/determinant;
                dxi_dx[4]=(dx_dxi[0]*dx_dxi[8] - dx_dxi[6]*dx_dxi[2])/determinant;
                dxi_dx[7]=-(dx_dxi[0]*dx_dxi[7] - dx_dxi[6]*dx_dxi[1])/determinant;

                dxi_dx[2]=(dx_dxi[1]*dx_dxi[5] - dx_dxi[4]*dx_dxi[2])/determinant;
                dxi_dx[5]=-(dx_dxi[0]*dx_dxi[5] - dx_dxi[3]*dx_dxi[2])/determinant;
                dxi_dx[8]=(dx_dxi[0]*dx_dxi[4] - dx_dxi[3]*dx_dxi[1])/determinant;
...
// ********************************************************
double Shape_function_2(int deg_of_freedom,int xi_index,int eta_index,int zeta_index,vector<double> Knot_Vector_xi,
                      vector<double> Knot_Vector_eta,vector<double> Knot_Vector_zeta,vector<double> dx_dxi)
{
    ...
    dxi_dtildexi.clear();
        ...
        dxi_dtildexi.push_back(0.5*(Knot_Vector_zeta[zeta_index+1]-Knot_Vector_zeta[zeta_index]));
    ...
    double determinant=0;
    determinant=dx_dtildexi[0]*dx_dtildexi[4]*dx_dtildexi[8]+dx_dtildexi[1]*dx_dtildexi[5]*dx_dtildexi[6]
                    +dx_dtildexi[2]*dx_dtildexi[3]*dx_dtildexi[7]-dx_dtildexi[6]*dx_dtildexi[4]*dx_dtildexi[2]
                    -dx_dtildexi[7]*dx_dtildexi[5]*dx_dtildexi[0]-dx_dtildexi[8]*dx_dtildexi[3]*dx_dtildexi[1];
return determinant;
}

// ********************************************************
//  COMPUTE DISPLACEMENT AND ITS DERIVATIVE W.R.T.
//  PHYSICAL COORDINATES
//                                  ** verified 04052021 **
// ********************************************************

void Shape_func_3(vector<int> ctrlpt_incidence,vector<double> ctrlpt_coordinates, vector<double>& NURBS_bas_func,vector<double>& NURBS_deriv,
                  vector<double> basis_func_N,vector<double> basis_func_M,vector<double> basis_func_L,vector<double> basis_deriv_N,
                  vector<double> basis_deriv_M,vector<double> basis_deriv_L,int order_p,int order_q,int order_r,int element,int nen,
                  int deg_of_freedom,vector<double>& du_dx,vector<double>& dx_dxi,vector<double> u_global_t,vector<double>& du_dxi)
{
    int loc_num=0;
    double sumtot=0;
    double sumxi=0;
    ...
    NURBS_deriv.clear();
        ...
        loc_num=0;
            ...
            dx_dxi.resize(deg_of_freedom*deg_of_freedom);
            du_dxi.clear();
            du_dxi.resize(deg_of_freedom*deg_of_freedom);
            ...
            for(int i=0;i<order_p+1;i++){
                ...
                for(int bb=0;bb<deg_of_freedom;bb++){
                        int cc=ctrlpt_incidence[nen*e+(loc_num-1)];
                        dx_dxi[deg_of_freedom*aa+bb]+=(ctrlpt_coordinates[4*(cc-1)+aa]*NURBS_deriv[deg_of_freedom*(loc_num-1)+bb]);
                        du_dxi[deg_of_freedom*aa+bb]+=(u_global_t[3*(cc-1)+aa]*NURBS_deriv[deg_of_freedom*(loc_num-1)+bb]);
                }}
            }}}
            ...
            //  Determine derivative of NURBS basis functions w.r.t physical coordinate
            du_dx.clear();
            du_dx.resize(nen*deg_of_freedom);

                for(int i=0;i<deg_of_freedom;i++)
                {
                    for(int j=0;j<deg_of_freedom;j++)
                    {
                        for(int k=0;k<deg_of_freedom;k++)
                        {
                            du_dx[deg_of_freedom*i+j]+=(du_dxi[deg_of_freedom*i+k]*dxi_dx[deg_of_freedom*k+j]);
                        }
                    }
                }
